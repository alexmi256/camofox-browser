...
describe('validateUrl() network safety', () => {
  ...
  test('does not report success when a slow navigation guard resolves private after the drain deadline', async () => {
    // The guard increments its in-flight counter, then awaits
    // validateNavigationUrl (a DNS lookup), and only records/aborts a blocked
    // target afterwards. A lookup slower than the drain deadline must not let
    // the action resolve first: that would report success before the block
    // verdict is known. It must fail boundedly and still free the tab lock.
    jest.useFakeTimers();
    try {
      lookupMock.mockImplementation(
      ...
      // Past the drain deadline and past the guard's 3.5s lookup.
  ...
  test('fails boundedly when an action-owned navigation guard never settles', async () => {
    // A guard that never resolves must not wedge the tab behind the drain: the
    // action fails instead of reporting an unknown-safety success, and the lock
    ...
    try {
      ...
      await createTabState(page);
      const action = withTabLock('wedged-guard-tab', () => withBlockedNavigationTracking(page, async () => {
        setTimeout(() => {
          void routeHandler(blockedRoute);
        }, 0);
        return 'result';
      ...
      let secondRan = false;
      await withTabLock('wedged-guard-tab', async () => {
        secondRan = true;
      });
      expect(secondRan).toBe(true);
    } finally {
